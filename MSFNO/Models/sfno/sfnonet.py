...
class Film_wrapper(nn.Module):
    def __init__(self,device,cfg):
        ...
        if self.cfg.film_gen_type == "gcn":
            ...
            self.film_gen = ViT(patch_size=self.cfg.patch_size, num_classes=num_film_features*self.cfg.film_layers*2, dim=self.cfg.embed_dim, depth=self.cfg.model_depth, heads=16, mlp_dim = self.cfg.mlp_dim, nan_mask_threshold=self.cfg.nan_mask_threshold, dropout = self.cfg.dropout, channels = self.cfg.temporal_step, device=self.device)
